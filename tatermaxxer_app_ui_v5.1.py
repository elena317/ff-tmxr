...
with advanced:
    ...
    c30, c31, c32, c33 = st.columns([1.15, 1.0, 1.0, 1.0])
    with c30:
        shape_3 = st.radio(
            "3-bag shape", ["Flat", "Squished"], horizontal=True,
            key="shape_3")
    with c31:
        flat_minimum_3_bag_width = st.number_input(
            "3-bag flat minimum width (in)", min_value=0.0,
            value=29.75, step=0.05)
    with c32:
        squished_minimum_3_bag_width = st.number_input(
            "3-bag squished minimum width (in)", min_value=0.0,
            value=29.75, step=0.05)
    with c33:
        inside_height_3 = st.number_input(
            "3-bag inside height (in)", min_value=0.01, value=2.0, step=0.05)
    minimum_3_bag_basket_width = (
        flat_minimum_3_bag_width
        if shape_3 == "Flat"
        else squished_minimum_3_bag_width
    )

    c20, c21, c22, c23 = st.columns([1.15, 1.0, 1.0, 1.0])
    with c20:
        shape_2 = st.radio(
            "2-bag shape", ["Flat", "Squished"], horizontal=True,
            key="shape_2")
    with c21:
        flat_minimum_2_bag_width = st.number_input(
            "2-bag flat minimum width (in)", min_value=0.0,
            value=19.8, step=0.05)
    with c22:
        squished_minimum_2_bag_width = st.number_input(
            "2-bag squished minimum width (in)", min_value=0.0,
            value=19.8, step=0.05)
    with c23:
        inside_height_2 = st.number_input(
            "2-bag inside height (in)", min_value=0.01, value=2.0, step=0.05)
    minimum_2_bag_basket_width = (
        flat_minimum_2_bag_width
        if shape_2 == "Flat"
        else squished_minimum_2_bag_width
    )

    c10, c11, c12, c13 = st.columns([1.15, 1.0, 1.0, 1.0])
    with c10:
        shape_1 = st.radio(
            "1-bag shape", ["Flat", "Squished"], horizontal=True,
            key="shape_1")
    with c11:
        flat_minimum_1_bag_width = st.number_input(
            "1-bag flat minimum width (in)", min_value=0.0,
            value=9.9, step=0.05)
    with c12:
        squished_minimum_1_bag_width = st.number_input(
            "1-bag squished minimum width (in)", min_value=0.0,
            value=9.9, step=0.05)
    with c13:
        inside_height_1 = st.number_input(
            "1-bag inside height (in)", min_value=0.01, value=2.0, step=0.05)
    minimum_1_bag_basket_width = (
        flat_minimum_1_bag_width
        if shape_1 == "Flat"
        else squished_minimum_1_bag_width
    )
