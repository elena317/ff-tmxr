...
with input_col:
    ...
    with st.expander("Vessel parameters", expanded=False):
        ...
        def flip_bag_dimensions():
            st.session_state["bag_length_saved"], st.session_state["bag_width_saved"] = (
                st.session_state["bag_width_saved"],
                st.session_state["bag_length_saved"],
            )
        ...
        st.button("Flip bag 180°", on_click=flip_bag_dimensions)
...
with advanced_settings_output:
    ...
    def persistent_number_input(
        label, saved_key, default_value, min_value, step
    ):
        ...
        # Streamlit removes the state of widgets that are not rendered. The saved
        # key is deliberately not attached to a widget, so it survives mode changes.
    ...
    def basket_class_controls(
        bag_class, flat_width_default, squished_width_default
    ):
        mode_col, width_col, height_col = st.columns([1.0, 1.15, 1.15])
        with mode_col:
        ...
        width_default = (
            flat_width_default if shape == "Flat" else squished_width_default
        )
    ...
    if tray_mode:
        st.markdown("**Individual tray geometry**")
        tray_col_1, tray_col_2 = st.columns(2)
        with tray_col_1:
        ...
        class_specs = {
            count: {
                "min_width": [0.0, minimum_1_bag_basket_width, minimum_2_bag_basket_width,
...
with input_3d_1:
    if tray_mode:
        ...
        basket_inside_length = st.number_input(
            "Basket inside length (in)",
            min_value=0.01, value=16.0, step=0.25
        )
with input_3d_2:
...
with input_3d_4:
    vessel_dead_volume_m = st.number_input(
        "Vessel dead volume (m)", min_value=0.0, value=0.3, step=0.1
    )
...
# ----------------------------
# Single-stack trolley visualization
# ----------------------------
...
st.subheader("Trolley Visualization")
st.caption(
    "One optimized stack shown inside its corresponding vessel section."
)
trolley_left, trolley_center, trolley_right = st.columns([0.10, 0.80, 0.10])
with trolley_center:
    st.plotly_chart(
        trolley_figure,
        use_container_width=True,
        config={"scrollZoom": True},
        key="trolley_visualization",
    )
