...
with advanced:
    ...
    class_header_1, class_header_2 = st.columns(2)
    class_header_1.caption("Minimum basket width (in)")
    class_header_2.caption("Inside height (in)")

    c31, c32 = st.columns(2)
    with c31:
        minimum_3_bag_basket_width = st.number_input(
            "3-bag minimum width", min_value=0.0, value=29.75, step=0.05,
            label_visibility="visible")
    with c32:
        inside_height_3 = st.number_input(
            "3-bag inside height", min_value=0.01, value=2.0, step=0.05,
            label_visibility="visible")

    c21, c22 = st.columns(2)
    with c21:
        minimum_2_bag_basket_width = st.number_input(
            "2-bag minimum width", min_value=0.0, value=19.8, step=0.05)
    with c22:
        inside_height_2 = st.number_input(
            "2-bag inside height", min_value=0.01, value=2.0, step=0.05)

    c11, c12 = st.columns(2)
    with c11:
        minimum_1_bag_basket_width = st.number_input(
            "1-bag minimum width", min_value=0.0, value=9.9, step=0.05)
    with c12:
        inside_height_1 = st.number_input(
            "1-bag inside height", min_value=0.01, value=2.0, step=0.05)
