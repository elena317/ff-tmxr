...
# Created here so these controls render below the complete 2D input/image row.
advanced_settings_output = st.container()
...
with input_col:
    ...
    load_system = st.radio(
        "Load carrier",
        ["Baskets", "Individual trays"],
        horizontal=True,
        help="The basket option uses the unchanged v5 engine. Individual trays use the separate v6 tray engine.",
    )
    tray_mode = load_system == "Individual trays"
    ...
    if tray_mode:
        basket_wall_thickness = 0.25
        tray_thickness = st.number_input(
            "Tray thickness (in)", min_value=0.0, value=0.25, step=0.05
        )
        tray_inside_height = st.number_input(
            "Individual tray inside height (in)",
            min_value=0.01,
            value=2.0,
            step=0.05,
        )
        inter_tray_gap = st.number_input(
            "Guide rail space (in)",
            min_value=0.0,
            value=0.0,
            step=0.05,
        )
        tray_inside_width = bag_width
        outside_tray_width = tray_inside_width + 2.0 * tray_thickness
        tray_outside_height = tray_inside_height + tray_thickness
        st.caption(
            f"Outside tray size used by the 2D solver: {outside_tray_width:.2f} in wide × "
            f"{tray_outside_height:.2f} in high. Trays are arranged side by side."
        )
        minimum_1_bag_basket_width = outside_tray_width
        minimum_2_bag_basket_width = 2.0 * outside_tray_width + inter_tray_gap
        minimum_3_bag_basket_width = 3.0 * outside_tray_width + 2.0 * inter_tray_gap
        inside_height_1 = inside_height_2 = inside_height_3 = tray_inside_height
        class_specs = {
            count: {
                "min_width": [0.0, minimum_1_bag_basket_width,
                              minimum_2_bag_basket_width,
                              minimum_3_bag_basket_width][count],
                "inside_height": tray_inside_height,
                "height": tray_outside_height,
            }
            for count in (1, 2, 3)
        }
        shape_1 = shape_2 = shape_3 = "Individual trays"
    else:
...
if tray_mode:
    solver_config = TrayOptimizerConfig(
        diameter=diameter,
        clearance=clearance,
        heater_height=heater_height,
        tray_inside_width=tray_inside_width,
        tray_inside_height=tray_inside_height,
        tray_thickness=tray_thickness,
        inter_tray_gap=inter_tray_gap,
        row_mode=row_mode,
        requested_rows=int(requested_rows) if requested_rows is not None else None,
    )
    solver_result = optimize_trays(solver_config)
else:
...
total_area = (
    sum(
        row["bags"] * (tray_inside_width + 2.0 * tray_thickness) * row["height"]
        for row in row_details
    )
    if tray_mode else
    sum(row["height"] * row["width"] for row in row_details)
)
active_wall_thickness = tray_thickness if tray_mode else basket_wall_thickness
carrier_singular = "tray" if tray_mode else "basket"
carrier_plural = "trays" if tray_mode else "baskets"
...
for idx, row in enumerate(row_details, start=1):
    ...
    if tray_mode:
        tray_outer_width = tray_inside_width + 2.0 * tray_thickness
        left = -width / 2.0
        for tray_index in range(bag_class):
            tray_left = left + tray_index * (tray_outer_width + inter_tray_gap)
            ax.add_patch(Rectangle(
                (tray_left, bottom_z),
                tray_outer_width, row_height,
...
ax.legend(handles=[green_basket, blue_basket, purple_basket],
          labels=(
              ["3-tray row", "2-tray row", "1-tray row"]
              if tray_mode else
              ["3 bag basket", "2 bag basket", "1 bag basket"]
          ),
          loc="upper right")
...
with optimal_stats_output:
    ...
    summary_cols[0].metric("Rows" if tray_mode else "Baskets", f"{n}")
    summary_cols[1].metric("Number of trays" if tray_mode else "Number of bags", f"{total_bags}")
    summary_cols[2].metric("Total tray-row area" if tray_mode else "Total basket area", f"{total_area:,.2f} in²")
...
if not tray_mode and simplify_manufacturing and rows:
...
def make_3d_snapshot():
    return {
        ...
        "n": int(n),
        "tray_mode": bool(tray_mode),
        "tray_outside_width": float(tray_inside_width + 2.0 * tray_thickness) if tray_mode else 0.0,
        "inter_tray_gap": float(inter_tray_gap) if tray_mode else 0.0,
...
def build_3d_figure(snapshot):
    ...
    tray_mode_3d = snapshot.get("tray_mode", False)
    tray_outside_width_3d = snapshot.get("tray_outside_width", 0.0)
    inter_tray_gap_3d = snapshot.get("inter_tray_gap", 0.0)
    ...
    for stack_idx in range(stack_count_3d):
        ...
        for bottom_z, width, row_height, bag_class in rows_3d:
            ...
            if tray_mode_3d:
                left = -width / 2.0
                for tray_index in range(bag_class):
                    x0 = left + tray_index * (tray_outside_width_3d + inter_tray_gap_3d)
                    add_box_to_group(
                        x0, x0 + tray_outside_width_3d,
                        bottom_z, bottom_z + row_height,
...
if view_mode == "Payload":
    ...
    input_3d_1, input_3d_2 = st.columns(2)

with input_3d_1:
    if tray_mode:
        basket_inside_length = float(bag_length)
        st.metric("Tray inside length", f"{basket_inside_length:.2f} in")
    else:
...
# Payload-only inputs retain their values while the Trolley view is selected.
st.session_state.setdefault("axial_stack_gap_3d_saved", 0.1)
st.session_state.setdefault("vessel_dead_volume_3d_saved", 0.3)
st.session_state.setdefault("vessel_length_3d_saved", 7.0)
st.session_state.setdefault("requested_axial_stacks_3d_saved", 15.0)
st.session_state.setdefault("specify_axial_stacks_3d_saved", False)
...
if view_mode == "Payload":
    ...
    with input_3d_3:
        st.session_state["_specify_axial_stacks_3d_widget"] = bool(
            st.session_state["specify_axial_stacks_3d_saved"]
        )

        def save_specify_axial_stacks():
            st.session_state["specify_axial_stacks_3d_saved"] = bool(
                st.session_state["_specify_axial_stacks_3d_widget"]
            )

        specify_axial_stacks = st.toggle(
            "Specify number of stacks",
            key="_specify_axial_stacks_3d_widget",
            on_change=save_specify_axial_stacks,
        )
    with input_3d_4:
    ...
    axial_stack_gap = float(st.session_state["axial_stack_gap_3d_saved"])
    specify_axial_stacks = bool(st.session_state["specify_axial_stacks_3d_saved"])
    vessel_dead_volume_m = float(st.session_state["vessel_dead_volume_3d_saved"])
...
if specify_axial_stacks:
    if view_mode == "Payload":
        ...
        requested_axial_stacks = float(st.session_state["requested_axial_stacks_3d_saved"])
    ...
    if view_mode == "Payload":
        ...
        vessel_length_m = float(st.session_state["vessel_length_3d_saved"])
...
def current_3d_signature():
    return (
        ...
        bool(tray_mode), round(float(tray_inside_width + 2.0 * tray_thickness), 10) if tray_mode else 0.0,
        round(float(inter_tray_gap), 10) if tray_mode else 0.0,
...
def snapshot_3d_signature(snapshot):
    return (
        ...
        bool(snapshot.get("tray_mode", False)),
        round(float(snapshot.get("tray_outside_width", 0.0)), 10),
        round(float(snapshot.get("inter_tray_gap", 0.0)), 10),
...
if view_mode == "Trolley":
    ...
    t1.metric("Rows per stack" if tray_mode else "Baskets per stack", f"{n}")
    t2.metric("Trays per stack" if tray_mode else "Bags per stack", f"{total_bags}")
    t3.metric(
        "Tray outside length" if tray_mode else "Basket outside length",
        f"{basket_outside_length_display:.2f} in",
    )
else:
    a1, a2, a3, a4, a5, a6, a7 = st.columns(7)
    a1.metric("Stacks", f"{stack_count_display}")
    a2.metric("Rows per payload" if tray_mode else "Baskets per payload", f"{total_vessel_baskets}")
    a3.metric("Trays per payload" if tray_mode else "Bags per payload", f"{total_vessel_bags}")
    a4.metric("Tray outside length" if tray_mode else "Basket outside length", f"{basket_outside_length_display:.2f} in")
...
st.session_state.setdefault("cycle_time_minutes_saved", 34.0)
if view_mode == "Payload":
    ...
    cycle_time_minutes = float(st.session_state["cycle_time_minutes_saved"])
capacity_mt_per_hr = (
...
# Detailed table
# ----------------------------
st.subheader("Tray-row dimensions and positions" if tray_mode else "Basket dimensions and positions")
...
st.dataframe(
    ...
    column_config={
        "Basket #": st.column_config.NumberColumn(
            "Row #" if tray_mode else "Basket #", width="small"
        ),
        ...
        "Bags": st.column_config.NumberColumn(
            "Trays" if tray_mode else "Bags", width="small"
        ),
        "Basket bottom Z (in)": st.column_config.NumberColumn(
            "Row bottom Z (in)" if tray_mode else "Basket bottom Z (in)",
...
def render_3d_snapshot_png(snapshot):
    ...
    tray_mode_3d = bool(snapshot.get("tray_mode", False))
    tray_width_3d = float(snapshot.get("tray_outside_width", 0.0))
    guide_space_3d = float(snapshot.get("inter_tray_gap", 0.0))
    ...
    for stack_index in range(stack_count_3d):
        ...
        for bottom_z, width, row_height, bag_class in rows_3d:
            ...
            if tray_mode_3d:
                left = -width / 2.0
                for tray_index in range(bag_class):
                    x0 = left + tray_index * (tray_width_3d + guide_space_3d)
                    add_box(
                        x0, x0 + tray_width_3d, y0, y1,
...
def build_configuration_pdf(include_3d=True, notes=""):
    ...
    with PdfPages(pdf_buffer) as pdf:
        ...
        geometry_items = (
            [
                ("Load carrier", "Individual trays"),
                ("Bag length (Y)", f"{bag_length:.2f} in"),
                ("Bag width (X)", f"{bag_width:.2f} in"),
                ("Individual tray outside width", f"{tray_inside_width + 2.0 * tray_thickness:.2f} in"),
                ("Individual tray outside length", f"{bag_length + 2.0 * tray_thickness:.2f} in"),
                ("Individual tray inside height", f"{tray_inside_height:.2f} in"),
                ("Individual tray outside height", f"{tray_inside_height + tray_thickness:.2f} in"),
                ("Tray thickness", f"{tray_thickness:.2f} in"),
                ("Guide rail space", f"{inter_tray_gap:.2f} in"),
            ]
            if tray_mode else
            [
        ...
        section("Results", [
            ("Rows per stack" if tray_mode else "Baskets per stack", n),
            ("Trays per stack" if tray_mode else "Bags per stack", total_bags),
            ("Stacks", stack_count_display),
            ("Rows per payload" if tray_mode else "Baskets per payload", total_vessel_baskets),
            ("Trays per payload" if tray_mode else "Bags per payload", total_vessel_bags),
        ...
        section("3D / payload configuration", [
            ("Tray inside length" if tray_mode else "Basket inside length", f"{basket_inside_length:.2f} in"),
            ("Tray outside length" if tray_mode else "Basket outside length", f"{basket_outside_length_display:.2f} in"),
            ...
            ("Cycle time", f"{cycle_time_minutes:.2f} min"),
            ("Tray-stack dead space" if tray_mode else "Basket dead space", f"{basket_dead_space:.2f} in"),
        ])
        note_y = max(y, 0.055)
